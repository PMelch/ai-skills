...
export class PiAgent extends BaseAgent {
  ...
  async isInstalled(): Promise<boolean> {
    try {
      await fs.access(join(this.homeDir, '.pi'));
      return true;
    } catch {
      return false;
    }
  ...
  async updateProjectConfiguration(skills: string[], projectRoot: string = process.cwd()): Promise<void> {
    const agentsConfigPath = join(projectRoot, 'AGENTS.md');

    let content = '';

    // Try to read existing file
    try {
      content = await fs.readFile(agentsConfigPath, 'utf-8');
    } catch {
    ...
    await fs.writeFile(agentsConfigPath, content, 'utf-8');
  }
